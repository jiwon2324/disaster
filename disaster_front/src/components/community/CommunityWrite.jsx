...
function CommunityWrite(){
  ...
  useEffect(()=>{
    // 🛡️ 비로그인 사용자가 주소창 입력을 통해 강제 진입했을 시 접근 방어 차단
  ...
  const handleSubmit = async (e) => {
    ...
    const vo = { title, content, writer, pw }
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3 mt-3">
          ...
          <input type="text" className="form-control" id="writer" placeholder="제보자명 입력" name="writer" required maxLength={10} onChange={(e) => setWriter(e.target.value)}/>
        </div>
        <div className="mb-3">
        ...
        <button type="button" className="btn btn-warning" onClick={() => navigate("/community/list")}>취소</button>
      </form>
    </>
  );
}
